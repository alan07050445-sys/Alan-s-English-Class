...
node tests/t-reading-background.mjs # 📖「讀之前先知道」：背景知識、圖片、繁體字、不爆雷、分段閱讀也能用、先講重點、簡報式呈現：57 項
node tests/t-flashcard-img.mjs   # 🃏 單字卡換圖不殘留上一張＋預先抓圖：10 項
```

十三支都不連網路（AI 也是假的）：Workers KV 用記憶體假的，Firestore 與 LINE API 的呼叫都被攔下來檢查內容（Flex Message 的結構也自己驗一遍，\nLINE 收到不合法的 JSON 會直接回 400）。
