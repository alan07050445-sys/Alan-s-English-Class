...
node tests/t-mascot-shop.mjs     # 🧸 吉祥物裝扮：商品表、買賣把關、星星怎麼扣、三個地方數字一致：39 項
```

十支都不連網路（AI 也是假的）：Workers KV 用記憶體假的，Firestore 與 LINE API 的呼叫都被攔下來檢查內容（Flex Message 的結構也自己驗一遍，\nLINE 收到不合法的 JSON 會直接回 400）。
